...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  const Vector3D& w_out = w2o * (-r.d);//You added r.o

  Spectrum L_out;
  // Spectrum black = Spectrum(1.0, 1.0, 1.0);
  ...
  for (SceneLight* sc : scene->lights)
  {
    ...
    {
      Spectrum sampleL = sc->sample_L(hit_p, &wi, &distToLight, &pdf);

      //comput w_in in object space
      Vector3D w_in = w2o * wi;

      //check if z coor of w_in is neg 
      if (w_in.z < 0)
      {

        //sampled light is lies behind surface so continue
        continue;
      }

      //cast shadow ray set max_t to distToLight, offset origin
      Vector3D shadow_o = EPS_D * wi + hit_p;
      Ray shadow_ray = Ray(shadow_o, wi);
      ...
      if (bvh->intersect(shadow_ray, &its))
      {
        continue;
      }
      else
      {
        //eval bsdf
        Spectrum L_i = isect.primitive->get_bsdf()->f(w_out, w_in);
        L_out += L_i*(sampleL*w_in.z/pdf);
      }
    }
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
  ...
  //Sample with isect.bsdf->sample_f()
  //maybe normalize w_in? Dont thinks so
  Vector3D w_in;
  float pdf;

  Spectrum out_rad = isect.bsdf->sample_f(w_out, &w_in, &pdf);

  //russian roulette, to terminate or to not terminate a ray that is the question
  // small term values will most likely return false if false we terminate ray
  // for checking difference in max ray depth remove  10
  float prob_term = out_rad.illum() * 10.0;
  prob_term = 1.0 - clamp(prob_term, 0.0, 1.0);

  if (coin_flip(prob_term))
  {
    //terminate
    return Spectrum();
  }
  ...
  {
    //remember to divide the spectrum value to be returned by  by 1 - prob_term 
    //create ray eps_d away from hitPt
    Vector3D wi = w2o.inv() * w_in;
    wi.normalize();
    Vector3D ray_o = EPS_D*wi + hit_p;
    Ray ray = Ray(ray_o, wi);
    ray.depth =  r.depth - 1;
    Spectrum t_spec = trace_ray(ray, isect.bsdf->is_delta());
    return (out_rad*t_spec * abs(w_in.z)) / (pdf * (1.0 - prob_term));
  }
...
//remove const
Spectrum PathTracer::trace_raypack(RayPacket &r, bool includeLe) {
  ...
  // If no intersection occurs, we simply return black.
  // This changes if you implement hemispherical lighting for extra credit.
  if (!bvh->intersect_raypack(r, &isect)) 
    return L_out;
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if (num_samples == 1)
  {
    ...
    //added for part 4 made camera ray depth max
...
void PathTracer::worker_thread() {
  ...
  if (continueRaytracing && workerDoneCount == numWorkerThreads) {
    ...
    fprintf(stdout, "[PathTracer] BVH traced %llu raypacks.\n", bvh->total_ray_packs);
