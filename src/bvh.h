...
class BVHAccel : public Aggregate {
 public:
  ...
  bool intersect(const Ray& rays, Intersection* i) const {
    ++total_rays;
    return intersect(rays, i, root);
  }
  

  //remove const
  bool intersect_raypack(RayPacket& raypack, BVHNode *node) const;
  //remove const
  bool intersect_raypack(RayPacket& raypack, Intersection* i, BVHNode *node) const;

  bool intersect_raypack(RayPacket& raypack, Intersection* i) const {
    ++total_ray_packs;
    return intersect_raypack(raypack, i, root);
  }
  ...
  mutable unsigned long long total_ray_packs;
 private:
